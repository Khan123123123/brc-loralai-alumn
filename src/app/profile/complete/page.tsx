  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        submitted_for_review: !currentlyApprovedInDb, 
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 rounded-3xl bg-primary p-8 text-white shadow-xl">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm"><ShieldCheck className="h-4 w-4" /> Member Profile</div>
      ...
      {banner && <div className={`mb-6 rounded-2xl border px-4 py-3 text-sm ${banner.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"}`}>{banner.text}</div>}
        ...
        <CardContent className="space-y-8">
          {step === 1 && (
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div><Label>Full name *</Label><Input value={formData.full_name} onChange={(e) => setField("full_name", e.target.value)} disabled={isAlreadyApproved} className={isAlreadyApproved ? "bg-slate-100" : ""} /></div>
              <div><Label>Roll number</Label><Input value={formData.roll_number} onChange={(e) => setField("roll_number", e.target.value)} /></div>
              <div>
                <Label>Entry year *</Label>
                <select className={`mt-2 h-10 w-full rounded-md border px-3 text-sm ${isAlreadyApproved ? "bg-slate-100 opacity-70 pointer-events-none" : ""}`} value={formData.entry_year} onChange={(e) => setField("entry_year", e.target.value)} disabled={isAlreadyApproved}>
                  <option value="">Select year</option>{Array.from({ length: 50 }, (_, i) => 1980 + i).map(y => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
              <div>
                <Label>Graduation year *</Label>
                <select className={`mt-2 h-10 w-full rounded-md border px-3 text-sm ${isAlreadyApproved ? "bg-slate-100 opacity-70 pointer-events-none" : ""}`} value={formData.graduation_year} onChange={(e) => setField("graduation_year", e.target.value)} disabled={isAlreadyApproved}>
                  <option value="">Select year</option>{Array.from({ length: 50 }, (_, i) => 1984 + i).map(y => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
              <div><Label>Student type</Label><select className="mt-2 h-10 w-full rounded-md border px-3 text-sm" value={formData.student_type} onChange={(e) => setField("student_type", e.target.value)}><option value="">Select</option>{studentTypes.map(i => <option key={i} value={i}>{i}</option>)}</select></div>
              <div><Label>Home district *</Label><select className="mt-2 h-10 w-full rounded-md border px-3 text-sm" value={formData.home_district} onChange={(e) => setField("home_district", e.target.value)}><option value="">Select</option>{districts.map(i => <option key={i} value={i}>{i}</option>)}</select></div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-8">
              <div className="grid gap-6 md:grid-cols-2">
                <div><Label>Current Position / Title *</Label><Input value={formData.current_position} onChange={(e) => setField("current_position", e.target.value)} /></div>
                <div><Label>Current Organization</Label><Input value={formData.current_organization} onChange={(e) => setField("current_organization", e.target.value)} /></div>
                <div><Label>Industry</Label><select className="mt-2 h-10 w-full rounded-md border px-3 text-sm" value={formData.industry} onChange={(e) => setField("industry", e.target.value)}><option value="">Select</option>{industries.map(i => <option key={i} value={i}>{i}</option>)}</select></div>
                <div><Label>Experience (years)</Label><Input type="number" value={formData.experience_years} onChange={(e) => setField("experience_years", e.target.value)} /></div>
              </div>

              <div className="border-t pt-6">
                <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-semibold text-primary">Higher Education</h3><Button type="button" variant="outline" size="sm" onClick={addEducation}><Plus className="w-4 h-4 mr-1"/> Add</Button></div>
                <div className="space-y-4">
                  {formData.higher_education.map((edu, index) => (
                    <div key={index} className="grid gap-4 md:grid-cols-2 bg-slate-50 p-4 rounded-xl relative border border-slate-200">
                      ...
                      <div><Label>Institution</Label><Input value={edu.institution} onChange={(e) => updateEducation(index, "institution", e.target.value)} /></div>
                      <div><Label>Degree</Label><Input value={edu.degree} onChange={(e) => updateEducation(index, "degree", e.target.value)} /></div>
                      <div><Label>Field of Study</Label><Input value={edu.field} onChange={(e) => updateEducation(index, "field", e.target.value)} /></div>
              ...
              <div className="border-t pt-6">
                <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-semibold text-primary">Job History</h3><Button type="button" variant="outline" size="sm" onClick={addJob}><Plus className="w-4 h-4 mr-1"/> Add</Button></div>
                <div className="space-y-4">
                  {formData.job_history.map((job, index) => (
                    <div key={index} className="grid gap-4 md:grid-cols-2 bg-slate-50 p-4 rounded-xl relative border border-slate-200">
                      ...
                      <div><Label>Company</Label><Input value={job.company} onChange={(e) => updateJob(index, "company", e.target.value)} /></div>
                      <div><Label>Title</Label><Input value={job.title} onChange={(e) => updateJob(index, "title", e.target.value)} /></div>
                      <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="flex items-center pt-8">
                        <label className="flex items-center gap-2 cursor-pointer text-sm"><Checkbox checked={job.is_current} onCheckedChange={(c) => updateJob(index, "is_current", Boolean(c))} /> I currently work here</label>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {step === 3 && (
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div><Label>LinkedIn URL</Label><Input className="mt-2 bg-white" value={formData.linkedin_url} onChange={(e) => setField("linkedin_url", e.target.value)} /></div>
