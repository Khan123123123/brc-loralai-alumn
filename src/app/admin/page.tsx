...
export default function AdminPage({ searchParams }: { searchParams: { search?: string } }) {
  ...
  useEffect(() => {
    const loadAdminData = async () => {
      ...
      if (!isAdmin) { window.location.replace("/directory"); return; }
  ...
  const handleSignOut = async () => { await supabase.auth.signOut(); window.location.replace("/auth/login"); };
  const deleteMessage = async (id: string) => { await supabase.from("contact_messages").delete().eq("id", id); setMessages(messages.filter(m => m.id !== id)); };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <Link href="/admin/announcements" className="inline-flex items-center gap-2 px-5 py-2 bg-primary text-white hover:bg-blue-900 rounded-xl text-sm font-bold shadow-sm"><Megaphone className="w-4 h-4" /> Manage Announcements</Link>
            <button onClick={handleSignOut} className="inline-flex items-center gap-2 px-5 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-red-50 hover:text-red-700 rounded-xl text-sm font-bold shadow-sm"><LogOut className="w-4 h-4" /> Sign Out</button>
          </div>
        </div>
                    ...
                    <tr key={profile.id} className="hover:bg-slate-50/50">
                      <td className="px-6 py-4 align-top w-1/4">
                        <div className="font-semibold text-slate-900 text-base flex items-center gap-2">
                           ...
                           {profile.featured_in_presentation && <Star className="w-4 h-4 fill-amber-400 text-amber-500" title="Featured Alumni"/>}
                        </div>
                        ...
                        <Link href={`/directory/${profile.slug || profile.id}`} target="_blank" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded"><ExternalLink className="w-3 h-3" /> View Profile</Link>
                      </td>
                        ...
                        <div className="flex flex-col items-end gap-3">
                          ...
                          <form action={toggleFeaturedStatus.bind(null, profile.id, profile.featured_in_presentation)}>
                             <button type="submit" className={`text-xs px-3 py-1.5 rounded-lg border shadow-sm font-bold flex items-center gap-1.5 ${profile.featured_in_presentation ? 'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                                <Star className={`w-3.5 h-3.5 ${profile.featured_in_presentation ? 'fill-amber-500 text-amber-500' : ''}`} /> {profile.featured_in_presentation ? "Unfeature" : "Feature Profile"}
                             </button>
                          </form>
                        </div>
                      </td>
