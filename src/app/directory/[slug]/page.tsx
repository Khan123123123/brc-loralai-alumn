...
export default async function DirectoryMemberPage({ params }: { params: { slug: string } }) {
  ...
  const { data: viewerProfile } = await supabase
    .from("profiles")
    .select("id, access_level, admin_status, verification_status")
    .eq("id", user.id)
    .single();
  ...
  if (error || !member || member.show_in_directory === false) {
    notFound();
  }
          ...
          <div className="flex items-start gap-5">
            ...
            <div>
              <div className="flex items-center gap-3">
                 ...
                 {member.featured_in_presentation && (
                   <span title="Featured Alumni">
                     <Star className="w-6 h-6 fill-amber-300 text-amber-300 drop-shadow-md" />
                   </span>
                 )}
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {member.account_type === "Faculty" && (
                  <Badge variant="outline" className="border-white/30 bg-white/10 text-white">
                    <BookOpen className="w-3 h-3 mr-1"/> Faculty
                  </Badge>
                )}
                ...
                {member.roll_number && (
                  <Badge variant="outline" className="border-white/30 bg-white/10 text-white">
                    Kit No: {member.roll_number}
                  </Badge>
                )}
                {member.regular_self_finance && (
                  <Badge variant="outline" className="border-white/30 bg-white/10 text-white">
                    {member.regular_self_finance}
                  </Badge>
                )}
                ...
                {isVerified ? (
                  <Badge className="bg-emerald-500 text-white hover:bg-emerald-600 border-0">
                    <ShieldCheck className="w-3 h-3 mr-1"/> Verified View
                  </Badge>
                ) : (
                  <Badge variant="secondary" className="bg-amber-500/80 text-white hover:bg-amber-600 border-0">
                    <Lock className="w-3 h-3 mr-1"/> Unverified
                  </Badge>
                )}
              </div>
              {isVerified && (member.current_position || member.profession) && (
                <p className="mt-4 text-white/90 text-lg font-medium">
                  {member.current_position || member.profession} {member.current_organization && `at ${member.current_organization}`}
                </p>
              )}
            </div>
          </div>
      ...
      {!isVerified && (
        <Card className="mb-8 rounded-3xl border-amber-200 bg-amber-50 shadow-sm">
          <CardContent className="flex flex-col gap-4 p-8 items-center text-center">
            ...
            <p className="text-amber-800 max-w-2xl leading-relaxed text-lg">
              Your account is currently Unverified. We protect professional histories, locations, and contact details for verified members only.
            </p>
            <Link href="/profile/complete" className="mt-4 rounded-full bg-primary px-8 py-3.5 text-sm font-bold text-white hover:bg-blue-900 shadow-lg transition-all">
              Complete Profile to Request Verification
            </Link>
          </CardContent>
        </Card>
      )}
          ...
          <div className="space-y-6 lg:col-span-2">
              ...
              <Card className="rounded-3xl border-0 shadow-md bg-white">
                <CardHeader className="pb-4 bg-slate-50 border-b border-slate-100">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <Briefcase className="w-5 h-5 text-slate-400"/> Work History
                  </CardTitle>
                </CardHeader>
                   ...
                   <div className="flex flex-wrap gap-4 mb-4 pb-4 border-b border-slate-100">
                      {member.employment_status && (
                        <div>
                          ...
                          <span className="font-semibold">{member.employment_status}</span>
                        </div>
                      )}
                      {member.experience_years && (
                        <div>
                          <span className="text-xs font-bold text-slate-400 uppercase block">Experience</span>
                          <span className="font-semibold">{member.experience_years} Years</span>
                        </div>
                      )}
                   </div>
            ...
            {member.higher_education && (member.higher_education as Education[]).length > 0 && (
              <Card className="rounded-3xl border-0 shadow-md bg-white">
                <CardHeader className="pb-4 bg-slate-50 border-b border-slate-100">
                  <CardTitle className="text-lg flex items-center gap-2">
                    <GraduationCap className="w-5 h-5 text-slate-400"/> Higher Education
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-6 space-y-5">
            ...
            {member.achievements && (
               <Card className="rounded-3xl border-0 shadow-md bg-white">
                 <CardHeader className="pb-4 bg-slate-50 border-b border-slate-100">
                   <CardTitle className="text-lg flex items-center gap-2">
                     <Award className="w-5 h-5 text-amber-500" /> Milestones & Memories
                   </CardTitle>
                 </CardHeader>
                ...
                <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-4 border border-slate-100">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-slate-900">
                      {member.current_city || "Unknown city"}{member.current_country ? `, ${member.current_country}` : ""}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl bg-emerald-50 p-4 border border-emerald-100">
                    ...
                    <div>
                      ...
                      <span className="font-bold text-emerald-900">
                        {member.home_city ? `${member.home_city}` : ""}{member.home_district ? ` (District ${member.home_district})` : ""}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <CardContent className="space-y-4 text-sm text-slate-700">
                {visibleContacts.email && (
                  <a href={`mailto:${visibleContacts.email}`} className="group flex items-center gap-4 rounded-2xl border border-slate-100 p-3 hover:bg-slate-50 transition-colors">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-primary group-hover:bg-primary/10 transition-colors">
